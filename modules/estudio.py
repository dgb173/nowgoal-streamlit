...
def get_match_details_from_row_of(row_element, score_class_selector="score", source_table_type="h2h"):
    try:
        ...
        home_idx, score_idx, away_idx, ah_idx, ou_idx = 2, 3, 4, 11, 15 # <--- CAMBIO CLAVE: Añadido ou_idx

        if len(cells) <= ou_idx: # Usamos el índice más alto para la validación
            return None

        # ... (extracción de fecha, equipos, marcador, etc. sin cambios) ...
        ...
        # <--- CAMBIO CLAVE: Extraer el resultado O/U de la última columna ---
        ou_result = "-"
        ...
        return {
            ...
            "ou_result": ou_result, # <--- CAMBIO CLAVE: Añadido al diccionario
...
def extract_h2h_data_of(soup, main_home_team_name, main_away_team_name, current_league_id=None):
    ...
    # 1. Extraer TODOS los enfrentamientos directos entre los dos equipos
    h2h_matches_all = []
    ...
    for r in rows:
        ...
        # Comprobar si es un H2H directo
    ...
    # Si no hay H2Hs, devolver vacío
    if not h2h_matches_all:
        return {"home_h2h": h2h_home_match_data, "latest_h2h": h2h_latest_match_data}

    # 2. Ordenar los H2H por fecha (más reciente primero)
    h2h_matches_all.sort(key=lambda x: _parse_date_ddmmyyyy(x.get("date", "")), reverse=True)

    # 3. Asignar el "H2H Último General" (siempre es el primero de la lista)
    h2h_latest_match_data = h2h_matches_all[0]

    # 4. Buscar el "H2H Directo (Local en casa)" más reciente
    for match in h2h_matches_all:
        if match["home"].lower() == main_home_team_name.lower():
            h2h_home_match_data = match
            break # Encontramos el más reciente, así que paramos
    ...
    if analizar_button_of:
        ...
        with st.spinner("🧠 Procesando datos y realizando análisis en paralelo..."):
            ...
            match_ids_to_fetch_stats = {
                ...
                'h2h_home': (h2h_home_match.get('matchIndex') if h2h_home_match else None,
                             h2h_home_match.get('home') if h2h_home_match else 'N/A',
                             h2h_home_match.get('away') if h2h_home_match else 'N/A'),
                'h2h_latest': (h2h_latest_match.get('matchIndex') if h2h_latest_match else None,
                               h2h_latest_match.get('home') if h2h_latest_match else 'N/A',
                               h2h_latest_match.get('away') if h2h_latest_match else 'N/A'),
            ...
            with st.expander("🔰 Hándicaps y Resultados Clave (H2H Directos)", expanded=True):
                ...
                with c1:
                    ...
                    if h2h_home_match:
                        display_previous_match_progression_stats(
                            f"H2H: {h2h_home_match.get('home')} vs {h2h_home_match.get('away')}",
                            h2h_home_match.get('matchIndex'), h2h_home_match.get('home'), h2h_home_match.get('away')
                        )
                with c2:
                     ...
                     if h2h_latest_match:
                        display_previous_match_progression_stats(
                            f"H2H: {h2h_latest_match.get('home')} vs {h2h_latest_match.get('away')}",
                            h2h_latest_match.get('matchIndex'), h2h_latest_match.get('home'), h2h_latest_match.get('away')
                        )
            st.divider()

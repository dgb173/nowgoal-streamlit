...
def check_handicap_cover(resultado_raw: str, ah_line_num: float, favorite_team_name: str, home_team_in_h2h: str, away_team_in_h2h: str):
...
def _analizar_precedente_handicap(precedente_data, ah_actual_num, favorito_actual_name):
    ...
    resultado_cover, cubierto = check_handicap_cover(res_raw, ah_actual_num, favorito_actual_name, home_team_precedente, away_team_precedente)
    ...
    comparativa_texto = f"El mercado mantiene una exigencia similar al {format_ah_as_decimal_string_of(ah_raw)} de entonces. "
    if ah_historico_num is not None:
        if exigencia_actual_fav < exigencia_historica_fav:
            comparativa_texto = f"El mercado ha **endurecido la línea** desde el {format_ah_as_decimal_string_of(ah_raw)} de aquel día. "
        elif exigencia_actual_fav > exigencia_historica_fav:
            comparativa_texto = f"El mercado ha **reducido la exigencia** desde el {format_ah_as_decimal_string_of(ah_raw)} de entonces. "
...
def _analizar_precedente_goles(precedente_data, goles_actual_num):
    ...
    try:
        ...
        cover_html = f"<span style='color: {'green' if 'SUPERADA' in resultado_cover else 'red'}; font-weight: bold;'>{resultado_cover}</span>"
...
def generar_analisis_completo_mercado(main_odds, h2h_data, home_name, away_name):
    ...
    sintesis_ah_estadio = _analizar_precedente_handicap(precedente_estadio, ah_actual_num, favorito_name)
    sintesis_goles_estadio = _analizar_precedente_goles(precedente_estadio, goles_actual_num)
    
    sintesis_ah_general = _analizar_precedente_handicap(precedente_general, ah_actual_num, favorito_name)
